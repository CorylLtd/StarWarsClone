...
Companion data: `trench-tables.json` (all 53 wedge tables row-by-row, the 11 pies + random pie, per-wave lengths, gun/catwalk counts, gun-probability and slot tables, force-bonus table, explosion timings, phase indices, RAM addresses). Conventions: decimal unless `0x`; frame = 20 Hz tick; cites `FILE:label`. In the trench all coordinates in the code are **real universe units** (the halving of `PRE2` is compensated by half-size models, see §3).
...
* **Shooting turrets** (WSLAZR.MAC:CLBLZ, LZCPNW, LZHPN): the laser is a 3-D ray from the ship to `(TX + 28672, TY + 7·RSX, TZ + 7·RSY)`. If it crosses a wall (`|Y| ≥ 1024`) before the floor: hit point on the wall → slot `(hitX >> 11) & 15`, X within the slot's 512..1536 (±64 laser radius), band by `hitZ + 4096` (first band edge at 128, 768 per band + 256 gaps); code **3 → turret destroyed, 100 pts** (`SCRTRT`), code **1 → panel destroyed, 50 pts** (`SCRPNL`); either sets `LZ.HIT = 4` (beam freezes, splash) and `AUDSX`; catwalks (2) cannot be shot. A fireball under the cursor is always hit first. If the ray reaches the floor (`Z = −4096`) it makes a green floor splash (`LZ.HIT = −1`) and may trigger the torpedo (§5.2).
...
There is no separate torpedo button. Every frame the lasers are on, the ray's floor intersection is computed: the far point is `(TX+28672, TY+7·RSX, TZ+7·RSY)`; if it is below the floor, `hitX = TX + 28672·(TZ+4096)/(−7·RSY)`, `hitY = TY + 7·RSX·(TZ+4096)/(−7·RSY)`, valid when `|hitY| ≤ 1024`. If the port exists (`BS.PFL`), `PT.LZF == 0`, `|hitY| ≤ 512` and `|hitX − BS.PLC| ≤ 512` → `PT.LZF = 1` → `FRPTGN`: `PT.LIV = 1`, all gun shots destroyed, music `PMSF2` + `AUDPH`. The torpedo pair (turquoise sparklers at `Y ± 128`) flies from `TX + 256` at 768 + VX per frame to `BS.PLC`, dives (`Z ≤ BS.PLC − X − 4096`) and glides to the midline. Once `PT.LIV` is set the hit is decided — the animation is cosmetic. Because the ray drops at most 7·104 = 728 units, the floor can only be reached when flying within 728 of it (`TZ < −3368`), and the hit lands `28672 · h / (7·|RSY|)` ahead (h = height above floor): e.g. 513 above the floor with full down-cursor → 20200 ahead. Shooting the port thus means: fly low, push the cursor down, and fire when the port is at that computed distance and within ±512 of the beam's lateral line.
